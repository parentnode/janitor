...
class User extends Model {
	...
	*/
	function __construct() {
		...
		$this->db_subscriptions = SITE_DB.".user_subscriptions";
					...
					if($query->sql($sql)) {
						...
						if($query->sql($sql)) {
							...
							if($subscription && $mail_password) {
								...
								$sql = "INSERT INTO $this->db_subscriptions SET user_id = $user_id, item_id = '$subscription', status = '1'";
								if($query->sql($sql)) {

									$IC = new Items();
									$item = $IC->getItem(array("id" => $subscription, "extend" => array("price" => true)));
									...
									$page->mail(array(
										"recipients" => $email, 
										"template" => "signup_subscription_error"
									));

									// send notification email to admin
									$page->mail(array(
										"subject" => "New subscription creation ERROR: " . $email, 
										"message" => "Do something", //"Check out the new user: " . SITE_URL . "/janitor/admin/user/edit/" . $user_id, 
										"template" => "system"
									));

									// add log
									$page->addLog("user->newUser (w/ subscription): creation failed: " . $email);
